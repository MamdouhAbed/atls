        ...
        <div class="content-header row">
          <div class="breadcrumb-wrapper col-xs-12">
            <ol class="breadcrumb">
              ...
              <li class="breadcrumb-item active">إضافة قسم وسائط فرعي
              </li>
            </ol>
          </div>
		...
		<div class="col-xs-12">
			<div class="card">
				<div class="card-header">
					<h4 class="card-title"><i class="icon-tag"></i> إضافة قسم وسائط فرعي</h4>
				...
				<div class="card-body collapse in">
					<form class="form card-block card-dashboard" novalidate="" action="<?php echo base_url().'admin/saveSubMediaDept'?>" method="post">

                      <div class="row">
                        <div class="form-group col-xs-6 mb-2">
                          <input type="text" id="eventRegInput1" class="form-control" placeholder="اسم القسم" required="" data-validation-required-message="أدخـل القسم" name="submedianame" />
                        </div>

                          <label class="col-md-1 " >يتبع قسم:</label>
                          <div class="col-md-3">
						...
						<table class="table table-striped table-bordered dataex-fixh-responsive">
							...
							<tbody>
                            <?php
                            foreach ($SubMedia as $submedia)
                            {?>
                                <tr>
                                    <td><?php  echo $submedia->id ?></td>
                                    <td><?php  echo $submedia->name ?></td>
                                    <td><?php  echo $submedia->main_name ?></td>
                                    <td><?php  echo $submedia->created_at ?></td>
                                    <td>

                                        <a class="btnupdate" data-toggle="modal" ><i class="fa fa-pencil info" title="Edit" style="font-size: 20px;"></i></a>
                  ...
                  <div class="modal fade text-xs-left" id="update" tabindex="-1" role="dialog" aria-labelledby="myModalLabel1" aria-hidden="true">
                      <div class="modal-dialog" role="document">
                          <div class="modal-content">
                              ...
                              <form action="<?php echo base_url().'admin/updateSubMediaDept'?>" method="Post" id="form-update" >
                              <div class="modal-body">
                                  <div class="row">
                                  <label class="col-md-4 text-xs-right control-label">اسم القسم : </label>
                                  <div class="col-sm-7">
...
<script>
    ...
    $(document).on('click','#save',function () {
        $.ajax({
            type: "POST",
            url: "<?php echo site_url('admin/updateSubMediaDept/');?>",
    ...
    $(document).on('click','#delete1',function () {
        $.ajax({
            type: "POST",
            url: "<?php echo site_url('admin/deleteSubMediaDept/');?>",
            data: {
                'deptID':$('#deptid2').val()},
            cache:false,
            success:
