...
<section id="responsive">
		...
		<div class="col-xs-12">
			<div class="card">
				<div class="card-header">
					<h4 class="card-title"><i class="icon-tag"></i> إضافة قسم رئيسي</h4>
				...
				<div class="card-body collapse in">
                    <form class="form card-block card-dashboard" novalidate="" action="<?php echo base_url().'admin/saveMainDept'?>" method="post">
                        <div class="form-body">
                            <div class="row">
                                <div class="form-group col-xs-6 mb-2">
                                    <input type="text" id="eventRegInput1" class="form-control" placeholder="اسم القسم" required="" data-validation-required-message="أدخـل القسم" name="mainnews" />
                                </div>
                            ...
                            <table class="table table-striped table-bordered dataex-fixh-responsive">
                                ...
                                <tbody>
                                <?php
                                foreach ($MainDept as $maindept)
                                {?>
                                    <tr>
                                        <td><?php  echo $maindept->id ?></td>
                                        <td><?php  echo $maindept->name ?></td>
                                        <td><?php  echo $maindept->created_at ?></td>
                                        <td>
          ...
          <div class="modal fade text-xs-left" id="update" tabindex="-1" role="dialog" aria-labelledby="myModalLabel1" aria-hidden="true">
              <div class="modal-dialog" role="document">
                  <div class="modal-content">
                      ...
                      <form action="<?php echo base_url().'admin/updateMainDept'?>" method="Post" id="form-update" >
    ...
    <script>
        ...
        $(document).on('click','#save',function () {
            $.ajax({
                type: "POST",
                url: "<?php echo site_url('admin/updateMainDept/');?>",
        ...
        $(document).on('click','#delete1',function () {
            $.ajax({
                type: "POST",
                url: "<?php echo site_url('admin/deleteMainDept/');?>",
                data: {
                    'deptID':$('#deptid2').val()},
                cache:false,
                success:
