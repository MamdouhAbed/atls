...
class Caricature extends CI_Controller {
    ...
    public function index()
    {
        ...
        $count = $this->db->count_all_results('caricature');
        $total_rows = 18;
        $config= Array();
        if ( $count > $total_rows ) {
            $config['base_url'] = base_url() . 'caricature/page';
            $config['per_page'] = 1;
        ...
        $data['Caricature'] = $this->CaricatureModel->get_caricature(18,$data['page']);
...
//        get break news
        ...
        $this->load->view('pages/caricature' , $data);
